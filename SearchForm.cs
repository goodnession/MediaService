...
namespace MediaService
{
    public partial class SearchForm : Form
    {
        ...
        public SearchForm(int userId)
        {
            ...
            command = new SqlCommand("SELECT TOP 50 a.AlbumCover, s.Song, s.SongName, s.SongId, a.AlbumName, a.AlbumId FROM Songs s INNER JOIN Album_Song aas ON s.SongId = aas.SongId INNER JOIN Albums a ON aas.AlbumId = a.AlbumId WHERE a.ReleaseDate <= GETDATE() ORDER BY NEWID()", db.getConnection());
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                songs.Controls.Add(new Label() { Text = "+", ForeColor = Color.FromArgb(0, 190, 10), AutoSize = true, Cursor = Cursors.Hand, Name = row["SongId"].ToString() }, 5, songs.RowCount - 1);
            }
        ...
        private void searchSongBtn_Click(object sender, EventArgs e)
        {
            ...
            string searchStr = "%" + searchBox.Text.Trim() + "%";

            songs.Controls.Clear();
            songs.RowCount = 1;
            ...
            SqlCommand command = new SqlCommand("SELECT u.UserName, a.AlbumCover, s.Song, s.SongName, s.SongId, a.AlbumName, a.AlbumId FROM Users u INNER JOIN Song_User su ON u.UserId = su.UserId INNER JOIN Songs s ON su.SongId = s.SongId INNER JOIN Album_Song aas ON s.SongId = aas.SongId INNER JOIN Albums a ON aas.AlbumId = a.AlbumId WHERE LOWER(s.SongName) LIKE(@sB) OR LOWER(a.AlbumName) LIKE(@sB) OR LOWER(u.UserName) LIKE(@sB) AND a.ReleaseDate <= GETDATE()", db.getConnection());

            command.Parameters.Add("@sB", SqlDbType.VarChar).Value = searchStr.ToLower();
