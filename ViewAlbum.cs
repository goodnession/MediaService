...
namespace MediaService
{
    public partial class ViewAlbum : Form
    {
        ...
        public ViewAlbum(int userId, int albumId)
        {
            InitializeComponent();
            ...
            command = new SqlCommand("SELECT a.AlbumCover, a.AlbumName, FORMAT(a.ReleaseDate, 'yyyy', 'en-US') AS ReleaseDate FROM Albums a WHERE a.AlbumId = @albId", db.getConnection());

            command.Parameters.Add("@albId", SqlDbType.Int).Value = this.albumId;
            ...
            albumCover.ImageLocation = outpath;
            albumName.Text = row["AlbumName"].ToString();
            releaseDate.Text = row["ReleaseDate"].ToString();

            table.Reset();
            command.Parameters.RemoveAt("@albId");

            command = new SqlCommand("SELECT u.UserName FROM Users u INNER JOIN Album_User au ON u.UserId = au.UserId WHERE au.AlbumId = @albId", db.getConnection());
            ...
            command = new SqlCommand("SELECT s.SongId, s.Song, s.SongName, s.ListeningCounter FROM Songs s INNER JOIN Album_Song als ON s.SongId = als.SongId WHERE als.AlbumId = @albId", db.getConnection());
            ...
            songs.AutoSize = true;
            songs.Font = new Font("Arial", 14, FontStyle.Bold);
            ...
            songs.Controls.Add(new Label() { Text = "Исполнитель", ForeColor = Color.Gray, AutoSize = true }, 2, 0);
            songs.Controls.Add(new Label() { Text = "Количество прослушиваний", ForeColor = Color.Gray, AutoSize = true }, 3, 0);
            songs.Controls.Add(new Label() { Text = "", ForeColor = Color.Gray, AutoSize = true }, 4, 0);
            ...
            for (int i = 0; i < table.Rows.Count; i++)
            {
                ...
                SqlCommand sqlCommand = new SqlCommand("SELECT u.UserName FROM Users u INNER JOIN Song_User su ON u.UserId = su.UserId WHERE su.SongId = @songId", db.getConnection());

                sqlCommand.Parameters.Add("@songId", SqlDbType.Int).Value = int.Parse(row["SongId"].ToString());
                ...
                songs.Controls.Add(new Label { Text = row["SongName"].ToString(), ForeColor = Color.White, AutoSize = true, Cursor = Cursors.Hand, Name = outpathSong }, 1, songs.RowCount - 1);
                songs.Controls.Add(new Label() { Text = arts, ForeColor = Color.White, AutoSize = true }, 2, songs.RowCount - 1);
                songs.Controls.Add(new Label() { Text = row["ListeningCounter"].ToString(), ForeColor = Color.White, AutoSize = true }, 3, songs.RowCount - 1);
                songs.Controls.Add(new Label() { Text = "+", ForeColor = Color.FromArgb(0, 190, 10), AutoSize = true, Cursor = Cursors.Hand, Name = row["SongId"].ToString() }, 4, songs.RowCount - 1);
            }
            ...
            for (int i = 6; i < songs.Controls.Count; i += 5)
            {
                songs.Controls[i].MouseClick += new MouseEventHandler(songClick);
            }

            for (int i = 4; i < songs.Controls.Count; i += 5)
            {
                songs.Controls[i].MouseClick += new MouseEventHandler(addSongToPlstClick);
            }
        }
        ...
        void songClick(object sender, MouseEventArgs e)
        {
            ...
            SqlCommand sqlCommand = new SqlCommand("UPDATE Songs SET ListeningCounter = ListeningCounter + 1 WHERE SongId = (SELECT s.SongId FROM Songs s INNER JOIN Album_Song aas ON s.SongId = aas.SongId WHERE s.SongName = @sgName AND aas.AlbumId = @albId)", dB.getConnection());

            sqlCommand.Parameters.Add("@sgName", SqlDbType.VarChar).Value = parent.Text;
            sqlCommand.Parameters.Add("@albID", SqlDbType.Int).Value = this.albumId;
            ...
            sqlCommand = new SqlCommand("DECLARE @songId INT SELECT @songId = s.SongId FROM Songs s INNER JOIN Album_Song aas ON s.SongId = aas.SongId WHERE s.SongName = @sgName AND aas.AlbumId = @albID INSERT INTO ListeningHistory VALUES (@usrID, @songId, GETDATE())", dB.getConnection());
            sqlCommand.Parameters.Add("@sgName", SqlDbType.VarChar).Value = parent.Text;
            sqlCommand.Parameters.Add("@albId", SqlDbType.Int).Value = this.albumId;
        ...
        void addSongToPlstClick(object sender, MouseEventArgs e)
        {
            Control parent = ((Control)sender);
            int songId = int.Parse((string)parent.Name);

            AddSongToPlaylistForm addSongToPlaylistForm = new AddSongToPlaylistForm(this.userId, songId);
            addSongToPlaylistForm.Show();
            if (addSongToPlaylistForm.flag)
                addSongToPlaylistForm.Close();
        }
